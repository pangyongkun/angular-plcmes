...
(function () {
    ...
    angular.module('raw-material-processing.single-fiber.capillary-batch')
        .controller('SFCapillaryBatchListCtrl', batchList)
        .controller('SFCapillaryBatchAddCtrl', batchAdd)
        .controller('SFCapillaryBatchMergeCtrl', batchMerge);


    batchList.$inject = ['SFCapillaryBatchService', 'progress', 'ConfirmDialog', 'notify', '$state', '$stateParams'];
    function batchList(service, progress, confirmDialog, notify, $state, $stateParams) {
        ...
        function paginationQuery(targetPage, itemsPerpage, params) {
            ...
            service.paginationQuery(targetPage, itemsPerpage, params).then(function (result) {
                ...
                vm.capillaryBatches = result.capillaryBatches;
                vm.isLoading = false;
            });
        }

        function deleteOne(capillaryBatch) {
            var messages = ['确认对象信息如下'];
            angular.forEach(capillaryBatch, function (value, key) {
                ...
                if (typeof (labs[key]) != 'object') {
                    ...
                    angular.forEach(capillaryBatch[key], function (_value, _key) {
                        if (labs[key][_key]) {
            ...
            modalInstance.result.then(function (flag) {
                if (flag == true) {
                    progress.start();
                    service.deleteOne(capillaryBatch.id).then(function (result) {
                        progress.stop();
                        notify({
                            message: '删除成功',
                            duration: 2000,//显示两秒
    ...
    batchAdd.$inject = ['SFCapillaryBatchService', '$state', '$stateParams', 'progress', 'notify'];
    function batchAdd(service, $state, $stateParams, progress, notify) {
        ...
        function saveSubmit() {
            ...
            progress.start();
            service.save(vm.capillaryBatch).then(function (result) {
                progress.stop();
                notify(
                    {
                        message: '添加批次成功',
    ...
    batchMerge.$inject = ['SFCapillaryBatchService', 'mergeSFCapillaryBatchPrepService', '$state', 'notify', 'progress'];
    function batchMerge(service, preService, $state, notify, progress) {
        ...
        vm.capillaryBatch = preService.capillaryBatch;
        ...
        function mergeSubmit() {
            progress.start();
            service.merge(vm.capillaryBatch).then(function (result) {
                progress.stop();
                notify(
                    {
                        message: '修改成功',
