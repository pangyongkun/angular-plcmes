...
(function () {
    angular.module('produce-process-pre-management.plan.order')
        .factory('PrePlanOrderService', prePlanOrderService);

    prePlanOrderService.$inject = ['$http', '$q', 'Restangular', '$timeout', '$state', '$stateParams'];


    function prePlanOrderService($http, $q, Restangular, $timeout, $state) {
        var randomsItems = [];
        var lastStateParams = '';
        var total = 666;
        for (var i = 0; i < 666; i++) {
        ...
        function back() {
            ...
            $state.go('produce-process-pre-management.plan.order.list', lastStateParams);
        }
        ...
        function paginationQuery(targetPage, pageItemsNum, params) {
            ...
            var orders = randomsItems.slice(start, end);
